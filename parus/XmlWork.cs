...
namespace parus
{
    ...
    {
        public int getIonogramMeasuringTime()
        {
            int time = 0; // ms
            string s_attr = "ionogram";
        ...
        public int getAmplitudesMeasuringTime()
        {
            int time = 0; // ms
            ...
            XmlElement xRoot = DocumentElement;
            // время для одного блока зондирования
            foreach (XmlNode Measurement in xRoot)
            {
                // находим атрибут name
                if (Measurement.NodeType == XmlNodeType.Element && 
                    Measurement.Attributes[0].Name == "name" && 
                    Measurement.Attributes[0].Value == "measurement")
                {
                    foreach (XmlNode module in Measurement)
                        if (module.NodeType == XmlNodeType.Element && 
                            module.Attributes[0].Name == "name" &&
                            module.Attributes[0].Value == "amplitudes")
                        {
                            time = Convert.ToInt32(module["dt"].InnerText);
                            break;
                        }
                }
